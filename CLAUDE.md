...
`AGENTS.md` is a symlink to `CLAUDE.md` at every level — same content, two filenames so any agent finds it. Per-package `CLAUDE.md` files refine these rules for a specific package; they may add detail but **must not contradict this file**. When in doubt, the root wins.
...
- **AI-legibility is first-class.** Identical signatures across V1/V2 where protocols overlap. Discriminated unions with obvious `type` tags. Deterministic outputs verifiable byte-for-byte. Error messages read like instructions an agent can act on without guessing. Protocol-specific terms (`LLTV buffer`, `wNative`, `GeneralAdapter1`, `bundler3`, `PublicAllocator`, `MetaMorpho`, `Permit2`, `WAD`) live in the [`packages/morpho-sdk/CLAUDE.md`](./packages/morpho-sdk/CLAUDE.md) glossary.
- **TypeDoc-generated reference** published per release.
- **Feedback loop:** if the same question is asked twice, the answer goes into the relevant `CLAUDE.md` or JSDoc on the export it concerns.
...
- If a package can't yet meet an applicable rule, keep the exception local and document why in the nearest `CLAUDE.md`.
