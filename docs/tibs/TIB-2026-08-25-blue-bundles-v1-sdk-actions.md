...
| **Scope**  | Package: `morpho-sdk` / next major release |
...
for the same user intent. The next major version instead makes it the single high-level write route
for Blue.
...
- Token approvals, ERC-2612 permits, Permit2 signatures, and Blue authorization target
  `BlueBundlesV1`.
...
This ships as a major bump of `@morpho-org/morpho-sdk`.
...
The release includes a migration guide and a major changeset. The previous major remains available
for products that still require the Bundler3 route; the new major does not carry both routes.
