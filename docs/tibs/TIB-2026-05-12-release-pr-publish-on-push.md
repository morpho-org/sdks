...
Concurrency is branch-scoped on the parent `push.yml` workflow. Feature-branch push runs can be cancelled by newer pushes. `main` and `next` runs are not cancelled, because interrupting a publish after npm succeeds but before tags or GitHub Releases are written would leave a partial release. The parent workflow serialization keeps release work ordered per branch, so rapid merges produce ordered runs: newer feature merges refresh the same release PR, while release-PR merge publishes remain isolated.
