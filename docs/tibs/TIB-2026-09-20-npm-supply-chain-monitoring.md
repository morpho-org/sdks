...
- Widening the policy requires a diff to `release-policy.json` in a reviewed PR.
- The verifier reads tarball metadata (`tar -t`, `tar -xO package/package.json`) and
  never executes package contents.
...
- [ ] Unit tests fail if any of these stop being rejected: non-allowlisted file, entry
      outside `package/`, install lifecycle script, `bin`, bundled dependencies,
      non-allowlisted dependency in any installed field, unexpected repository URL,
      unlisted package.
- [ ] Local `pnpm build:ci && pnpm pack` of every public package passes the verifier.

## Consequences

- Adding a runtime/peer dependency or a shipped non-`lib` file now requires a
  `release-policy.json` change in the same PR. This is intended friction.
