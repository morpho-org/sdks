...
- **Phase 2b — Test migration to Vitest.** Port the unit suite first: colocate as `src/**/*.test.ts`, drop every `jest.unstable_mockModule('viem' | '@morpho-org/morpho-sdk' | …)` call, and rewrite "SDK called with X" assertions as `Transaction` decode assertions. Then port the integration suite to `test/**/*.integration.test.ts` on `@morpho-org/test` (`createViemTest` / `createAnvilTestClient`), pin the mainnet block, and swap the hardcoded mnemonic for the harness's prefunded accounts and ERC-20 storage seeding helpers. Add fast-check property tests on the requirement translator. Add separate unit and fork Vitest project entries for the package in the root `vitest.config.ts`, then retire the Phase-1 jest step from `test.yml`.
