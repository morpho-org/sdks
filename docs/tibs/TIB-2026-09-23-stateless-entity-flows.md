...
- [ ] Every value `buildTx()` derives from a signature is read from `signature.args`; every
      re-derivable identity field is validated against the handle's immutable inputs and rejected
      with a typed error on mismatch, every signed bound is validated against the fresh derivation
      in the direction the contract enforces (lower bound for funding caps), and every opaque
      payload is presence-checked with its sibling `args` validated.
