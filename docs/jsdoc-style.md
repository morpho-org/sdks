...
- **Changesets follow semver relevance** per [`AGENTS.md`](../AGENTS.md) §7. JSDoc-only changes, repo-meta-only PRs (TIB, style guide, root tooling), and tests-only changes do not need a changeset unless they accompany a behavior-affecting published package source change.
