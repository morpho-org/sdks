[**@morpho-org/sdks**](../../../README.md)

***

[@morpho-org/sdks](../../../README.md) / [morpho-sdk/src](../README.md) / BlueSupplyCollateralBorrowAction

# Interface: BlueSupplyCollateralBorrowAction

Defined in: [packages/morpho-sdk/src/types/action.ts:204](https://github.com/morpho-org/sdks/blob/000d92bc88f6b9370d16dcc3069ffd81fbd85fe8/packages/morpho-sdk/src/types/action.ts#L204)

## Extends

- [`BaseAction`](BaseAction.md)\<`"blueSupplyCollateralBorrow"`, \{ `borrowAmount`: `bigint`; `collateralAmount`: `bigint`; `market`: `Hex`; `minSharePrice`: `bigint`; `nativeAmount?`: `bigint`; `onBehalf`: `Address`; `reallocationFee`: `bigint`; `reallocationPenaltyAssets`: `bigint`; `receiver`: `Address`; \}\>

## Properties

### args

> `readonly` **args**: `object`

Defined in: [packages/morpho-sdk/src/types/action.ts:14](https://github.com/morpho-org/sdks/blob/000d92bc88f6b9370d16dcc3069ffd81fbd85fe8/packages/morpho-sdk/src/types/action.ts#L14)

#### borrowAmount

> **borrowAmount**: `bigint`

#### collateralAmount

> **collateralAmount**: `bigint`

#### market

> **market**: `` `0x${string}` ``

#### minSharePrice

> **minSharePrice**: `bigint`

#### nativeAmount?

> `optional` **nativeAmount?**: `bigint`

#### onBehalf

> **onBehalf**: `` `0x${string}` ``

#### reallocationFee

> **reallocationFee**: `bigint`

Native-token fees paid to PublicAllocator V1.

#### reallocationPenaltyAssets

> `readonly` **reallocationPenaltyAssets**: `bigint`

Loan-token assets donated as BluePublicAllocator V2 penalties.

#### receiver

> **receiver**: `` `0x${string}` ``

#### Inherited from

[`BaseAction`](BaseAction.md).[`args`](BaseAction.md#args)

***

### type

> `readonly` **type**: `"blueSupplyCollateralBorrow"`

Defined in: [packages/morpho-sdk/src/types/action.ts:13](https://github.com/morpho-org/sdks/blob/000d92bc88f6b9370d16dcc3069ffd81fbd85fe8/packages/morpho-sdk/src/types/action.ts#L13)

#### Inherited from

[`BaseAction`](BaseAction.md).[`type`](BaseAction.md#type)
