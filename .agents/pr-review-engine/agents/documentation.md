...
- **Accepted ADR rewrites.** Treat an ADR already present on the target branch as a historical record. Flag edits that update its implementation-time names or examples; the only permitted edits to an accepted ADR are its Status row (`superseded by <stem>` or `accepted; <part> superseded by <stem>`), its filename, or a link path whose target was renamed. Changed decisions require a new superseding ADR under `docs/adrs/` that lists the old record in its References. Flag any new `docs/tibs/` file or `TIB-*` filename: TIBs are retired and `docs/tibs/` no longer exists. An ADR introduced on the current branch may stay in sync with its implementation before landing.
