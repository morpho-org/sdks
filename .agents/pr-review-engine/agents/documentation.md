...
- `docs/**/*.md` (style guides, architecture deep-dives, ADRs, retired TIBs, templates).
...
- **Accepted ADR and implemented TIB rewrites.** Treat an ADR or TIB already present on the target branch as a historical record. Flag edits that update its implementation-time names or examples; the only permitted edit to an accepted ADR is flipping its Status row to `superseded by`, and a TIB is never edited. Changed decisions require a new superseding ADR under `docs/adrs/` that lists the old record in its References. Flag any new file under `docs/tibs/`: TIBs are retired. An ADR introduced on the current branch may stay in sync with its implementation before landing.
