...
- Internal workspace dependencies that do not use `workspace:^`, except two authorized cases. (1) `peerDependencies` intentionally use explicit published semver ranges so Changesets does not auto-bump peer dependents. When a package is bumped, check all packages that declare it as a peer dependency; flag missing peer range updates or missing explicit dependent changesets. (2) The §4 *leaf funds-handling adapter exception* permits `workspace:*` (exact pin) on the **direct** transaction-building deps of a leaf adapter — one nothing else in the monorepo depends on that constructs value-bearing transactions. A `workspace:*` pin is only acceptable when the package qualifies as such a leaf adapter and records the deviation, rationale, obligations, and limits in its own `AGENTS.md`; flag any `workspace:*` that lacks those criteria or that documentation. When it does qualify, the exact pin obliges a coordinated patch of the adapter on every bump of a pinned dep (§7 maintained-dependent audit) — flag a bump of a pinned dep whose exact-pinning dependent is not released in the same changeset, since the pin freezes rather than drifts forward.
