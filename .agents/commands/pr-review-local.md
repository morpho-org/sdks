...
- `<LINT_CMD>` = `pnpm exec biome check <changed files>` — scoped to the diff's files. NOT the full `pnpm lint` (it bundles `jsdoc:coverage:check` + `lint:address` + `typecheck:scripts` (the `tsc` type-check of `scripts/` via `scripts/tsconfig.json`) + a whole-repo `biome check` that is pre-existing-red on frozen deprecated packages).
- `<TYPECHECK_CMD>` = `tsc --noEmit` scoped to the touched surface: `.agents/pr-review-engine/scripts/tsconfig.json` for engine-script changes, `scripts/tsconfig.json` (`pnpm typecheck:scripts`) for `scripts/` changes (including `scripts/ci/` and `scripts/release/`), or the touched package's `tsconfig.json` (`pnpm --filter <pkg> exec tsc --noEmit`). Skip with a one-line warning if no tsconfig covers the change.
- `<TEST_CMD>` = the affected **non-watch** Vitest project only: `dotenv -- node node_modules/vitest/vitest.mjs run --project <name>` (e.g. `agents-engine` for engine-script changes; `scripts` for `scripts/ci/` or `scripts/release/` changes; the touched package's project otherwise). **Never** bare `pnpm test` (watch mode) and **never** the full fork suite (needs `MAINNET_RPC_URL` + live RPC).
