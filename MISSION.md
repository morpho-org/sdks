...
2. **Secure, tested & audited.** Security is the posture; tests and audits are the evidence. We codify invariants as tests, pin ABIs and addresses in-package, audit every major release, and treat threat-model review as routine when new protocol surface lands. (Test/audit specifics: see [`CLAUDE.md`](./CLAUDE.md) §5 and §7.)
