...
- The repo is **public** and the release pipeline (`push.yml`) triggers on `push` to every branch (`branches: ['**']`). GitHub does **not** expose repository secrets to workflow runs from **fork** pull requests, and a fork cannot trigger a `push` on the base repo — so every secret below is reachable only by actors with **write access** (humans + trusted bots). Fork and Dependabot PRs receive no secrets (`claude.yml` gates its secret-bearing run to same-repo, non-fork, non-Dependabot heads).
