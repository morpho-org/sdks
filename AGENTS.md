...
- **Accepted ADRs and implemented TIBs are historical records.** Do not rewrite a record already present on the target branch to follow later code, symbol, or path changes. Keep its implementation-time names and examples intact. A changed decision gets a new superseding ADR that lists the old record in its References; when the old record is an ADR its Status is flipped to `superseded by`. TIBs are retired: no new TIB is written, and a superseded TIB is never edited — its supersession is recorded solely in the new ADR's References. Only an ADR introduced for the current implementation may be kept in sync with that implementation before it lands.
