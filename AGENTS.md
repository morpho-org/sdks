...
- **CI logic is tested code.** Any `run:` block beyond a single command with no branching (parsing API responses, filtering, counting, computing outputs, deciding pass/fail) lives in a TypeScript script under `scripts/ci/` (run as `node <script>.ts`, Node ≥ 22.18 strips types natively — new release scripts should follow suit rather than adding `.mjs`) with colocated `*.test.ts` in the `scripts` project, and the workflow step just invokes it. Inline `jq`/`awk`/shell pipelines encoding such logic are **high**: they run untested until the workflow fires in production and their failures are invisible in review (see `scripts/ci/claude-review-gate.ts` for the pattern: I/O injected via `fetchImpl`/`env`, pure filtering exported and unit-tested). Steps that must run against the PR head execute the trusted default-branch copy of the script, never the workspace one.
