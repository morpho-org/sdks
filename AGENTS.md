...
- **Accepted ADRs are historical records.** Do not rewrite a record already present on the target branch to follow later code, symbol, or path changes. Keep its implementation-time names and examples intact — the records migrated from `docs/tibs/` keep theirs. The only permitted edit to an accepted record is flipping its Status row to `superseded by`; a changed decision gets a new superseding ADR that lists the old record in its References. Only an ADR introduced for the current implementation may be kept in sync with that implementation before it lands.
