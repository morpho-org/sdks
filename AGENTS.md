...
- **Workspace deps** use workspace ranges (`"@morpho-org/blue-sdk": "workspace:^"`), except internal `peerDependencies`. Internal peers use explicit published semver ranges (`"^6.0.0"`, not `"workspace:^"`) so Changesets does not auto-bump peer dependents. When a package is bumped, the author and reviewer must audit every package that declares it as a peer dependency, decide whether its peer range must be updated, and include explicit changesets for every affected dependent package. Do not rely on Changesets to infer peer-dependent releases.
  - **Leaf funds-handling adapter exception.** A leaf integration adapter that nothing else in the monorepo depends on, and that constructs value-bearing transactions (calldata, `value`, approvals, recipients) before forwarding them to a writable wallet account, may pin its direct transaction-building Morpho workspace deps with `workspace:*` (published as the exact tested version) instead of `workspace:^`, so a passive semver minor/patch of a direct dep cannot alter transaction shape without a deliberate release of the adapter. The exception is limited to direct deps — it does not freeze the transitive graph, which stays on its own caret ranges — and it obliges the adapter to ship a coordinated patch on every bump of a pinned dep (§7 maintained-dependent audit), since an exact pin freezes rather than drifts. Each use must be recorded in the package's `AGENTS.md` with rationale, obligations, and limits. Today's sole instance: `wdk-protocol-lending-morpho-evm` (SDKS-72).
