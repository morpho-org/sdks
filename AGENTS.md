...
- **EVM simulation Tenderly retirement exception (approved 2026-09-24).** `evm-simulation` 5.0.0 may remove `TenderlyRpcConfig`, `ChainSimulationConfig.tenderlyRpc`, and the Tenderly/provider-fallback behavior without the successor-introduction, `@deprecated`, and one-minor coexistence steps: the sole supported backend is `eth_simulateV1`, so a coexistence minor would only keep a retired provider reachable. It does not waive the major changeset, migration notes, the maintained-dependent audit, or continued availability of the previous major. No other API removal inherits this exception.
