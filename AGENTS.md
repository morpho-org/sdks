...
- **Peer range audit on package bumps.** Internal peer dependency ranges are maintained manually. Every changeset that bumps a package must be checked against downstream `peerDependencies`; if a dependent package should accept the new version or no longer accepts the old compatibility set, update its peer range and include that dependent package in the changeset with the appropriate bump.
