...
- **Registered-secret diagnostics.** Do not flag unmodified subprocess diagnostics solely because they can repeat an exact GitHub Actions secret when every in-repo workflow passes that value through `secrets.*` and `env:`; GitHub masks registered secret values in workflow logs. This applies only to logs: transformed or partial values, unregistered credentials, and uploaded artifacts still require separate protection.
