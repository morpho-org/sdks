...
- **Accepted ADRs and implemented TIBs are historical records.** Do not rewrite a record already present on the target branch to follow later code, symbol, or path changes. Keep its implementation-time names and examples intact. A changed decision gets a new superseding ADR that lists the old record in its References and flips the old record's Status. TIBs are retired: no new TIB is written and existing ones under `docs/tibs/` are not edited. Only an ADR introduced for the current implementation may be kept in sync with that implementation before it lands.
