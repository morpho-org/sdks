...
- **EVM simulation Tenderly retirement exception.** The removal defined by [SDK-1291](https://linear.app/morpho-labs/issue/SDK-1291/pr-17-retire-tenderly-and-make-eth-simulatev1-the-sole-backend) (approved 2026-09-24; the simulation TIB on `tib-tx-safety-skills` carries the dated addendum) means `evm-simulation` 5.0.0 may remove `TenderlyRpcConfig`, `ChainSimulationConfig.tenderlyRpc`, and the Tenderly/provider-fallback behavior without the successor-introduction, `@deprecated`, and one-minor coexistence steps: the sole supported backend is `eth_simulateV1`, so a coexistence minor would only keep a retired provider reachable. It does not waive the major changeset, migration notes, the maintained-dependent audit, or continued availability of the previous major. No other API removal inherits this exception.
