...
| [`.agents/skills/execute-project/SKILL.md`](./.agents/skills/execute-project/SKILL.md) | `/execute-project` — runs a Linear project end to end: one child agent per issue, stacked draft PRs in dependency waves, each child following `/create-pr` and the `/review-pr-gh` → `/fix-pr` loop. Symlinked into `.claude/skills/`. |
