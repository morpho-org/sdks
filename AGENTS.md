...
- **AI-legibility is first-class.** Identical signatures across V1/V2 where protocols overlap. Discriminated unions with obvious `type` tags. Deterministic outputs verifiable byte-for-byte. Error messages read like instructions an agent can act on without guessing. Protocol-specific terms (`LLTV buffer`, `wNative`, `BlueBundlesV1`, `BluePublicAllocator`, `MetaMorpho`, `Permit2`, `WAD`) live in the [`packages/morpho-sdk/AGENTS.md`](./packages/morpho-sdk/AGENTS.md) glossary.
- **Technical decisions are ADRs.** A hard-to-revert decision or a new technical standard is recorded as an Architectural Decision Record under [`docs/adrs/`](./docs/adrs/) from [`docs/templates/ADR.md`](./docs/templates/ADR.md), accepted through its own PR and frozen after merge; only its Status field changes. Planning content (milestones, acceptance criteria, owners, file paths) lives in the Linear Technical Project Plan, not the ADR. See [`docs/DEVELOPMENT-LIFECYCLE.md`](./docs/DEVELOPMENT-LIFECYCLE.md).
- **Accepted ADRs are historical records.** Do not rewrite a record already present on the target branch to follow later code, symbol, or path changes. Keep its implementation-time names and examples intact — the records migrated from `docs/tibs/` keep theirs. The only permitted edits to an accepted record are its Status row (`superseded by <stem>` or `accepted; <part> superseded by <stem>`), its filename, or a link path whose target was renamed; a changed decision gets a new superseding ADR that lists the old record in its References. Only an ADR introduced for the current implementation may be kept in sync with that implementation before it lands.
...
- **BlueBundlesV1 route exception.** The route replacement defined by [`ADR-2026-08-25-blue-bundles-v1-sdk-actions`](./docs/adrs/ADR-2026-08-25-blue-bundles-v1-sdk-actions.md) may skip the prior successor-introduction, `@deprecated`, and one-minor coexistence steps: `morpho-sdk` 6.0.0 may replace the listed Blue methods' Bundler3 implementations and remove or retype the listed route-specific inputs and outputs while preserving the established method and action names; `wdk-protocol-lending-morpho-evm` 2.0.0 may make the Blue input changes listed in that record. Vault V1 reallocations are excluded from this exception: their high-level Blue and WDK flows must be deprecated in a published minor before the new majors accept only Vault V2 reallocations. The exception is otherwise limited to that route and those inputs and outputs because carrying both high-level routes would preserve an ambiguous product choice. It does not waive major changesets, migration guides, the maintained-dependent audit and bumps, or continued availability of the previous majors. No break outside this route inherits this exception.
- **Vault V2 `forceWithdraw` route exception.** The route replacement defined by [`ADR-2026-08-28-vault-exit-force-withdraw`](./docs/adrs/ADR-2026-08-28-vault-exit-force-withdraw.md) may likewise skip the successor-introduction, `@deprecated`, and one-minor coexistence steps: `morpho-sdk` 6.0.0 may replace `MorphoVaultV2.forceWithdraw` and the `vaultV2ForceWithdraw` action's `VaultV2.multicall` implementation with the `VaultExitBundlesV1` periphery and retype their inputs and `VaultV2ForceWithdrawAction.args`, while preserving the established method and action names. The exception is granted for fund-safety: the prior multicall path carried **no slippage bound at all** — a share-price drop, penalty increase, or liquidity shift executed silently at the worse terms — so a coexistence minor would keep that fund-loss surface exported and reachable (the record's rejected Alternative 2). Its `{ deallocations, withdraw }` input shape is also structurally incompatible with the penalty-inclusive `{ exitAssets, vaultData }` shape, and `forceRedeem` is untouched and retains the multicall path for multi-adapter and legacy-adapter vaults. It does not waive the major changeset, the migration guide entry, the maintained-dependent audit, or continued availability of the previous major. No break outside this route inherits this exception.
- **Bundler3-primitive removal exception.** The removal defined by [`ADR-2026-09-17-remove-bundler3-primitives-without-deprecation`](./docs/adrs/ADR-2026-09-17-remove-bundler3-primitives-without-deprecation.md) may likewise skip the successor-introduction, `@deprecated`, and one-minor coexistence steps: `morpho-sdk` 6.0.0, `wdk-protocol-lending-morpho-evm` 2.0.0, `morpho-ts` 3.0.0, `blue-sdk` 7.0.0, and `blue-sdk-viem` 6.0.0 may remove the never-deprecated low-level Bundler3 and migration-adapter surfaces listed in that record — the `./bundler` subpath and `BundlerAction`, Bundler3 executor/adapter ABIs, addresses, and registry trees, `getGeneralAdapterRequirements*`, `Holding.permit2BundlerAllowance`, `User.isBundlerAuthorized`, the five v5 partial-refinance error classes, and the compatibility symbols first deprecated only during the prereleases. The exception is granted because no SDK product composes Bundler3 calls once every high-level route is served by the standalone bundle contracts, so a coexistence minor would only mark symbols with no supported successor. It does not waive major changesets, migration guides, the maintained-dependent audit and bumps, or continued availability of the previous majors. No break outside this scope inherits this exception.
...
- **CI logic is tested code.** Any `run:` block that derives a decision or a value from data — parsing API responses, filtering or counting records, computing step outputs, deciding pass/fail — lives in a TypeScript script under `scripts/ci/` (run as `node <script>.ts`, Node ≥ 22.18 strips types natively — new release scripts should follow suit rather than adding `.mjs`) with colocated `*.test.ts` in the `scripts` project, and the workflow step just invokes it. Inline `jq`/`awk`/shell pipelines encoding such logic are **high**: they run untested until the workflow fires in production and their failures are invisible in review. Exempt: linear setup sequences (install, `sysctl`, `cp`), a single command with a static error message on failure, and marshalling static workflow inputs into command arguments — no data is interpreted, so there is nothing to unit-test (see `scripts/ci/claude-review-gate.ts` for the pattern: I/O injected via `fetchImpl`/`env`, pure filtering exported and unit-tested). Steps that must run against the PR head execute the trusted default-branch copy of the script, never the workspace one.
...
- **Secret scoping & branch-gating.** Each workflow secret's intended reach (repo-level/every-branch vs branch-gated vs OIDC/`environment`-gated) and sensitivity are recorded in [`.github/workflows/AGENTS.md`](./.github/workflows/AGENTS.md); a new secret name added to a workflow must gain a row there in the same PR. Widening a secret's reachable scope is a finding: removing or loosening the `if: github.ref_name == 'main' || github.ref_name == 'next'` gate on the `version-pr`/`publish` jobs, moving a write/publish/signing secret into the ungated `test` job (every-branch), or exposing any secret to a fork-accessible trigger is **high** — **critical** for the write-token or publish path. A low-sensitivity, write-access-only secret already repo-level on every branch (the RPC URLs) is the accepted baseline: flag a *regression* of these gates, not the baseline itself.
...
- **Artifact identity: ask the consumer, don't emulate it** (path-injection defense). When a privileged job checks an artifact produced by an unprivileged job (e.g. a `.tgz` from the pack job), the value it validates must be read through the *same code path the consumer will use*. For npm tarballs that means `pacote.manifest("file:<tgz>", { fullMetadata: true, fullReadJson: true })` from the npm bundled with the pinned Node (`scripts/ci/read-tarball-identity.ts`) — the exact call `npm publish` makes — never a literal `tar -x package/package.json` or a `tar -t | grep` pass, and never a hand-rolled tar/PAX/path parser that *predicts* what node-tar will do. Archive entries such as `package/\./package.json`, `./package/package.json`, or a second root (`zzz/package.json`) are normalized differently by GNU tar and node-tar, so any check that reads the archive with a different tool than the consumer can be made to see a benign manifest while npm publishes a malicious one (SDK-672/673). Cheap structural checks with GNU tar (canonical paths, single `package/` root, regular files only) are fine as defense in depth but never as the identity source of truth. Reviewers flag new archive/path-normalization logic under `scripts/ci/` as **high**: the fix is to reuse the toolchain's own reader, not to extend the emulation. The one sanctioned normalization is `scripts/ci/verify-tarball-collisions.ts`, which lists entries with npm's bundled node-tar (`strict: true`) and rejects entries a case-insensitive/Windows consumer *filesystem* would fold together (case, `\`, `.` segments, trailing dots/spaces, `~`, non-ASCII, reserved names) — it models the consumer's filesystem, never the archive format. It also models pacote's `.gitignore` → `.npmignore` rename during extraction. The literal `package/package.json` may still be *validated* (e.g. the `publishConfig` allowlist in `scripts/ci/verify-tarball-manifest.ts`, SDK-664) as long as the workflow asserts its `name@version` equals the pacote-resolved identity, which proves the validated manifest is the one npm publishes.
...
Retired in-repo workflows — use the shared skills from morpho-org/internal-skills instead:
decisions: `work-loop:write-adr`; project plans: `work-loop:write-plan`; tickets from a plan:
`work-loop:extract-plan`; retro: `work-loop:reflect`.
