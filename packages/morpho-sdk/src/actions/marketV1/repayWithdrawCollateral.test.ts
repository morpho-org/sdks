...
describe("marketV1RepayWithdrawCollateral unit tests", () => {
  ...
  test("should create repay-by-assets + withdraw collateral transaction", async ({
    ...
    expect(tx.action.type).toBe("marketV1RepayWithdrawCollateral");
  ...
  test("should append metadata to transaction data when provided", async ({
    ...
    const txWith = marketV1RepayWithdrawCollateral({
      market: {
        chainId: mainnet.id,
        marketParams: WethUsdsMarketV1,
      },
    ...
    expect(txWith.action.type).toBe("marketV1RepayWithdrawCollateral");
  });
});
