...
/**
 * Builds a Permit2 SignatureTransfer requirement for a registered fixed-bundles token pull.
 *
 ...
 * @param params.spender - Registered VaultBundlesV1 or BlueBundlesV1 deployment that performs the pull.
 ...
 * @throws {UnsupportedErc20ApprovalSpenderError} when `spender` is not a supported fixed-bundles deployment for `chainId`.
...
export const encodeErc20Permit2TransferFrom = (
  ...
  // metadata that the bundles action verifies. Direct callers bypass the token-requirement resolver, so
