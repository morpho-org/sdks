...
2. Encode calldata. **Bundler3 paths** use `BundlerAction.encodeBundle`. **Blue write paths** encode
   ...
   directly. Vault `inKindRedeem` and `vaultV2/forceWithdraw` actions encode a standalone
   `VaultExitBundlesV1` entry point rather than composing a Bundler3 bundle; `vaultV2/forceRedeem`
   stays on `VaultV2.multicall`.
