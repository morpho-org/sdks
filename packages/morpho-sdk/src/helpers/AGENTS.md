...
- **Shared-liquidity** — `computeVaultV1Reallocations` builds PublicAllocator V1 reallocations for a borrow/withdraw; `computeReallocations` remains its deprecated compatibility alias. `computeVaultV2Reallocations` plans BluePublicAllocator reallocations and applies the configured WAD-scaled penalty threshold in both discovery phases. `getSupplyTargetUtilization(marketId, options)` resolves the per-market → default → `DEFAULT_SUPPLY_TARGET_UTILIZATION` supply target for V1. Read-only liquidity metrics live on the corresponding versioned reallocation-data entity, not in this layer.
