...
- **Validators** (pure, throw typed errors) — `validateVaultV2BlueReallocations(...)`, `validateSlippageTolerance(...)`, `validatePositionHealth(...)`. Each enforces a public-API invariant: see the `error.ts` exports for the full list of error classes a caller may pattern-match on.
- **Math / share-price helpers** — helpers for vault share-price bounds use
  `MAX_SLIPPAGE_TOLERANCE` and cap at `MAX_ABSOLUTE_SHARE_PRICE`. The high-level Blue write methods
  do not use these helpers or accept slippage inputs.
