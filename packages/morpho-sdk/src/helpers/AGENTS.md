...
- **Shared-liquidity** — `computeReallocations` builds PublicAllocator reallocations for a borrow/withdraw; its `maintainSupplyTargetUtilization` option holds the target market at `supplyTargetUtilization` instead of relaxing it to 100% in the aggressive phase (source markets are still drained to supply that liquidity). The read-only metric `computeAvailableSharedLiquidity` (a standalone helper, not a `MorphoBlue` method) never throws on insufficiency — it returns `0n` when no liquidity is available.
