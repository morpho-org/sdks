...
Entry point of the SDK. `MorphoClient` wraps a viem **`PublicClient`** (exported from this package: `Client<Transport, Chain>`) and exposes vault/market accessors. Inherits the rules in [`packages/morpho-sdk/AGENTS.md`](../../AGENTS.md).

## Client contract

- Input is `PublicClient` (alias for `Client<Transport, Chain>` — chain mandatory). Account is not read.
- Used for: on-chain reads, building transactions, fetching nonces / token data.
- Signing happens elsewhere: pass a `WalletClient` to `Requirement.sign(client, userAddress)` (see [`src/actions/AGENTS.md`](../actions/AGENTS.md)).
