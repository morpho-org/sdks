...
**Design rationale** (cf. [ARCHITECTURE.md](ARCHITECTURE.md#withdrawals-vaultbundlesv1-redeems-direct-vault-calls)): a withdraw does not transfer tokens _from_ the user to the protocol, it burns shares. There is therefore no _inflation_ attack surface to close — hence a single call outside Bundler3 (to VaultBundlesV1, not the vault directly), simpler from a UX standpoint than a full bundle. **But the trade-off is real**: the absence of an on-chain share-price check still leaves the caller exposed to share-price manipulation, to be weighed case by case.
