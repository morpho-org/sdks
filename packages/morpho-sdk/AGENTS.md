...
- **VaultBundlesV1** — standalone periphery that funds and executes a VaultV1 or VaultV2 deposit in one call. It is the ERC-20 approval spender and the Permit2/ERC-2612 permit spender for every vault deposit, pulls the gross assets itself, deducts the optional referral fee, and enforces `maxSharePrice` and `deadline`. Registered per chain as `bundles.vaultBundlesV1`; its canonical ABI export is `vaultBundlesV1Abi`.
