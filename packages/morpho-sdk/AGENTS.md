...
- **GeneralAdapter1** — the bundler-side adapter that holds approvals/auth and executes Morpho calls on the user's behalf on **Bundler3** routes. Now used only by explicit low-level Bundler3 composition. Every high-level Blue write — including `refinance` — approves and authorizes **BlueBundlesV1** directly.
