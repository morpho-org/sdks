...
- **Blue position paths** (`supplyCollateral`, `borrow`, `supplyCollateralBorrow`, `repay`, `withdrawCollateral`, `repayWithdrawCollateral`) each encode one direct `BlueBundlesV1` **combined** call — `blueBundlesV1SupplyCollateralAndBorrow` for the collateral-supply/borrow family, `blueBundlesV1RepayAndWithdrawCollateral` for the repay/collateral-withdraw family — with the inactive leg zeroed for single-sided actions. Loan-asset `supply` and `withdraw` encode one direct `BlueBundlesV1` call each (`blueBundlesV1Supply` / `blueBundlesV1Withdraw`). `repay` and `withdraw` each accept assets or shares (mutually exclusive); `repayWithdrawCollateral` repays first then withdraws. Native funding is supported by collateral supply (`collateralToken === wNative`), loan-asset `supply`, `repay`, and `repayWithdrawCollateral` (`loanToken === wNative`) — attached to the single payable BlueBundlesV1 call as `tx.value`. It is all-or-nothing (`validateBlueBundlesV1NativeFunding` requires `nativeAmount` to equal the full funded amount: `collateralAssets` for collateral supply, `assets` for loan-asset `supply`, and the derived `maxRepayAssets` cap — repay assets/shares plus referral fee — for `repay` / `repayWithdrawCollateral`), so a native-funded call pulls no ERC-20 and there is no additive or share-carve mixed funding. `borrow` / `supplyCollateralBorrow` and loan-asset `withdraw` carry optional Vault V2 BluePublicAllocator reallocations inside the combined calldata, with penalties netted from the borrow or withdrawn proceeds.
...
- **GeneralAdapter1** — the bundler-side adapter that holds approvals/auth and executes Morpho calls on the user's behalf on **Bundler3** routes. Now used only by `refinance` (the ERC-20 approval spender and authorized Morpho operator for its bundle) and by explicit low-level Bundler3 composition. The migrated Blue position paths and loan-asset `supply` / `withdraw` no longer use it — they approve and authorize **BlueBundlesV1** directly.
...
- **`setAuthorization`** — Morpho call that grants an operator the right to call market functions on behalf of the user. Required pre-condition for `borrow`, `supplyCollateralBorrow`, `repayWithdrawCollateral`, `withdrawCollateral`, and loan-asset `withdraw`, which all authorize **BlueBundlesV1** (embedded in the combined-call calldata); `refinance` authorizes **GeneralAdapter1** through the same Morpho mechanism.
- **`setAuthorizationWithSig`** — the offchain-signature equivalent of `setAuthorization`. When the client opts into signatures (`supportSignature: true`), the authorization requirement becomes a signable `Requirement`; the signed `AuthorizationRequirementSignature` removes the standalone authorization transaction. The BlueBundlesV1 combined calls and loan-asset `withdraw` embed the signed authorization struct directly in their calldata; `refinance` instead folds a `setAuthorizationWithSig` call into its Bundler3 bundle (prepended before the Morpho operation).
