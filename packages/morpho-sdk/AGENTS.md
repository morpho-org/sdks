...
- **Blue bundled paths** (`supplyCollateral`, `borrow`, `supplyCollateralBorrow`, `repay`, `repayWithdrawCollateral`) route through bundler3 via GeneralAdapter1. Loan-asset `supply` and `withdraw` instead encode one direct `BlueBundlesV1` call each (`blueBundlesV1Supply` / `blueBundlesV1Withdraw`). `repay` and `withdraw` each accept assets or shares (mutually exclusive); `repayWithdrawCollateral` repays first then withdraws. Native wrapping is supported by loan-asset `supply` (a direct payable BlueBundlesV1 call funded by `tx.value`), `repay`, and `repayWithdrawCollateral` when `loanToken === wNative` (repay assets mode is additive like supply; repay shares mode carves native out of the transfer). Loan-asset `withdraw` supports optional Vault V2 BluePublicAllocator reallocations carried inside the `blueBundlesV1Withdraw` calldata, with penalties netted from the withdrawn proceeds.
...
- **GeneralAdapter1** — the bundler-side adapter that holds approvals/auth and executes Morpho calls on the user's behalf. Required as the spender for ERC-20 approvals on every bundled path; required as authorized operator on Morpho for `borrow`, `supplyCollateralBorrow`, and `repayWithdrawCollateral`. Loan-asset `withdraw` authorizes **BlueBundlesV1** instead.
...
- **`setAuthorization`** — Morpho call that grants GeneralAdapter1 the right to call market functions on behalf of the user. Required pre-condition for `borrow`, `supplyCollateralBorrow`, and `repayWithdrawCollateral`; loan-asset `withdraw` authorizes **BlueBundlesV1** through the same Morpho mechanism (embedded in the `blueBundlesV1Withdraw` call).
- **`setAuthorizationWithSig`** — the offchain-signature equivalent of `setAuthorization`. When the client opts into signatures (`supportSignature: true`), the authorization requirement becomes a signable `Requirement`; the signed `AuthorizationRequirementSignature` is folded into the bundle as a `setAuthorizationWithSig` call (prepended before the Morpho operation), removing the standalone authorization transaction. Loan-asset `withdraw` instead embeds the signed authorization struct directly in the `blueBundlesV1Withdraw` calldata.
