...
`inKindRedeem` and `forceWithdraw` use it too, so callers can await `getRequirements()` to check
...
direct calls — vault `withdraw` / `redeem` and `forceRedeem` — have no prerequisites and return only
`{ buildTx }`.
