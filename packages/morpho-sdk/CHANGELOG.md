...
- Updated dependencies [[`b7c60ed`](https://github.com/morpho-org/sdks/commit/b7c60ed7c7b278caed30c13ec57eedc28d63380f)]:
  - @morpho-org/midnight-sdk@1.8.0-next.0
...
- [#990](https://github.com/morpho-org/sdks/pull/990) [`b8c944c`](https://github.com/morpho-org/sdks/commit/b8c944c44251c48985cb7411310884eb2548208c) Thanks [@Foulks-Plb](https://github.com/Foulks-Plb)! - Route the established Blue refinance flow through BlueBundlesV1 as a full compatible-position
...
- Route the referral-fee, deadline, and ECDSA-signature guards shared by the direct BlueBundlesV1 writes and the VaultExitBundlesV1 force withdrawal through single helpers instead of per-call-site copies.
  ...
  The referral-fee failure mode is one canonical class — `ReferralFeeRecipientMissingError` — because both peripheries transfer the fee unconditionally. `validateReferralFee` owns the `[0, WAD)` bound (throwing `ReferralFeePctExceededError`) plus the recipient check, `validateDeadline` owns the positive-`uint256` deadline bound, and the shared `normalizeBundlesSignature` owns the 64-byte EIP-2098 / 65-byte parse and the `yParity` → `v` widening for every periphery permit and authorization struct. The three remaining hand-rolled `uint256` bounds (`blueSupply`'s `assets`, and the fixed-bundles token requirement resolver's `amount` and `permit2Nonce`) now call the existing `validateUint256Field` like their siblings.

  Beyond the referral-fee error's canonical rename and removal of its legacy alias, encoded calldata and requirement shapes are unchanged, with one deliberate behavior addition: `vaultV1InKindRedeem` and `vaultV2InKindRedeem` had no upper bound on `deadline` at all, so routing them through the shared guard means an out-of-`uint256` deadline now throws `InputExceedsMaxError` instead of viem's `IntegerOutOfRangeError` at encode time, matching their `vaultV2ForceWithdraw` sibling on the same periphery.
...
- [#1067](https://github.com/morpho-org/sdks/pull/1067) [`f5f0acb`](https://github.com/morpho-org/sdks/commit/f5f0acbb5c9d05dd08f64e411884d1b89f5c876e) Thanks [@Foulks-Plb](https://github.com/Foulks-Plb)! - Expose the EIP-712 payload of every signature `Requirement` as `action.typedData`, so integrators
  ...
  Each signable requirement action — ERC-2612 permit (`PermitAction`), Permit2 SignatureTransfer
  (`Permit2SignatureTransferAction`), Morpho authorization (`AuthorizationAction`), and the Midnight
  offer-root (`MidnightOfferRootSignatureAction`) — now carries a `typedData` field holding the exact
  viem `TypedDataDefinition` that `sign()` signs. On the `Requirement` returned by `getRequirements()`
  ...
  `encodeErc20Permit` and `encodeBlueSignatureAuthorization`. Their `sign()` now also rejects a signer
  that differs from `owner`. All in-SDK requirement resolvers already supply the owner, so high-level
...
- [#997](https://github.com/morpho-org/sdks/pull/997) [`c58f155`](https://github.com/morpho-org/sdks/commit/c58f155d46c89338727c7783bfbeec4ed255f23e) Thanks [@jinmel](https://github.com/jinmel)! - Generalize the fixed-bundles token requirement surface shared by BlueBundlesV1 and VaultBundlesV1,
  ...
  Reject out-of-range uint256 pull amounts in the standalone requirement resolver. Expose the
  canonical `BundleSharesPermit` tuple as the single shares-permit shape (no compatibility aliases).

  Update the WDK's public token requirement signatures to `BundlesTokenRequirementSignature` from
  the new morpho-sdk major.

- [#1000](https://github.com/morpho-org/sdks/pull/1000) [`5b2009e`](https://github.com/morpho-org/sdks/commit/5b2009eb27dbe6bed0b1a10dacb3715b3ac64c91) Thanks [@jinmel](https://github.com/jinmel)! - Route Vault V1 and Vault V2 share redemptions and Vault V1-to-V2 migrations through VaultBundlesV1 with exact share requirements, referral fees, deadlines, and destination price protection.
...
- [#1073](https://github.com/morpho-org/sdks/pull/1073) [`21e557c`](https://github.com/morpho-org/sdks/commit/21e557cc81ba1a7d638975010c1bb74e75943f5f) Thanks [@Foulks-Plb](https://github.com/Foulks-Plb)! - Bound Vault V2 force-withdraw `minSharePriceE27` overrides by the SDK's maximum-slippage floor so overrides cannot disable meaningful share-price protection.
...
- [#1054](https://github.com/morpho-org/sdks/pull/1054) [`91a6e29`](https://github.com/morpho-org/sdks/commit/91a6e291c8bf7d1c919436eba7bede023cabe8aa) Thanks [@jinmel](https://github.com/jinmel)! - Reuse the shared bundles permit converter for VaultExitBundlesV1 in-kind redemptions with strict
  permit validation.
...
- [#1072](https://github.com/morpho-org/sdks/pull/1072) [`000afdd`](https://github.com/morpho-org/sdks/commit/000afddd06a2bd55d22b452985975d4b02a541e8) Thanks [@Foulks-Plb](https://github.com/Foulks-Plb)! - Full repay now defaults its classic approval to the exact `maxRepayAssets` pull instead of the token's reusable maximum or `maxUint256`; pass `approvalAmount` to keep a reusable allowance.
...
- [#1071](https://github.com/morpho-org/sdks/pull/1071) [`85a0932`](https://github.com/morpho-org/sdks/commit/85a09327c5a269153bbda94b83f30efd4493c45a) Thanks [@Foulks-Plb](https://github.com/Foulks-Plb)! - Require exact vault-share allowances for Vault V1 and Vault V2 in-kind redemptions, replacing oversized allowances with onchain approvals and strictly validating embedded permits.
...
- [#1158](https://github.com/morpho-org/sdks/pull/1158) [`b52411c`](https://github.com/morpho-org/sdks/commit/b52411c837d15f576eb653d8ed84cabb6c6800d9) Thanks [@Foulks-Plb](https://github.com/Foulks-Plb)! - Harden vault reads and the vault-shares permit encoder:
...
- Updated dependencies [[`0e72b04`](https://github.com/morpho-org/sdks/commit/0e72b0439aa46c7a7d6b4e6fad6d2d9c79c2e45e), [`8d74feb`](https://github.com/morpho-org/sdks/commit/8d74feb2ca41210d70fb0a593641da4a4994e350)]:
  - @morpho-org/morpho-ts@3.0.0-next.0
  - @morpho-org/blue-sdk@7.0.0-next.1
  - @morpho-org/blue-sdk-viem@6.0.0-next.0
  - @morpho-org/midnight-sdk@1.5.1-next.0
...
- [#1056](https://github.com/morpho-org/sdks/pull/1056) [`c4b4467`](https://github.com/morpho-org/sdks/commit/c4b44677e7a6881072eca0fe5eba54c3d9761b60) Thanks [@Foulks-Plb](https://github.com/Foulks-Plb)! - Correct Midnight taker examples to preserve the caller's requested asset target and explicit unit guard while forwarding the complete fallback offer list.
...
- Updated dependencies [[`c4b4467`](https://github.com/morpho-org/sdks/commit/c4b44677e7a6881072eca0fe5eba54c3d9761b60), [`c4b4467`](https://github.com/morpho-org/sdks/commit/c4b44677e7a6881072eca0fe5eba54c3d9761b60), [`c4b4467`](https://github.com/morpho-org/sdks/commit/c4b44677e7a6881072eca0fe5eba54c3d9761b60)]:
  - @morpho-org/blue-sdk-viem@5.5.1-next.0
  - @morpho-org/midnight-sdk@1.4.0-next.0
...
- [#939](https://github.com/morpho-org/sdks/pull/939) [`007eebf`](https://github.com/morpho-org/sdks/commit/007eebf49ca9e67039eeac1445a5c74f7a64841f) Thanks [@Rubilmax](https://github.com/Rubilmax)! - Include Vault V2 per-market rounding headroom in bounded in-kind redeem share permits and approvals.
