...
export const mine = async (blocks = 1, options: { interval?: number } = {}, emit = true) => {
  const { ethers } = require("hardhat") as HardhatRuntimeEnvironment;

  await hardhatMine(blocks, options);

  if (emit) await ethers.provider.emit("block", await ethers.provider.getBlockNumber());
};
...
export const resetAfterEach = (forking?: { url?: string | null; blockNumber?: number }) => {
  afterEach(async () => {
    await reset(forking);
  });
};
