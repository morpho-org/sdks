...
export type DottedKeys<T> = T extends object | null | undefined
  ? {
      [K in keyof T & string]: T[K] extends object | null | undefined ? `${K}.${DottedKeys<T[K]>}` : K;
    }[keyof T & string]
  : "";

export type PartialDottedKeys<T> = T extends object | null | undefined
  ? {
      [K in keyof T & string]: T[K] extends object | null | undefined ? `${K}.${PartialDottedKeys<T[K]>}` | K : K;
    }[keyof T & string]
  : "";
