...
export const bigIntComparator =
  <T>(getter: (x: T) => bigint | undefined | null, order: "asc" | "desc" = "asc") =>
  (a: T, b: T) => {
    const xA = getter(a);
    const xB = getter(b);
...
export const getValue = <T, Path extends PartialDottedKeys<T> = PartialDottedKeys<T>>(data: T, path: Path) =>
  _get(data, path.split(".")) as FieldType<T, Path>;

export const transformValue = <T, R>(value: T | null | undefined, _transform: (v: T) => R) =>
...
export const keys = <T>(o?: T) => Object.keys(o ?? {}) as (T extends ArrayLike<unknown> ? number : keyof T)[];

export const values = <T>(o?: T) => Object.values(o ?? {}) as (T extends ArrayLike<infer U> ? U : T[keyof T])[];

export const entries = <T>(o?: T) =>
  Object.entries(o ?? {}) as [keyof T, T extends ArrayLike<infer U> ? U : T[keyof T]][];

export const fromEntries = <K extends PropertyKey, T>(srcEntries: Iterable<readonly [K, T]>) =>
