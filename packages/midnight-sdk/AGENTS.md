...
- Pure protocol helpers live in `market/`, `offers/`, `math/`, `bundles/`, `calls/`, and `signatures/`. Chain reads live only in `fetch/`; router HTTP calls live only in `router/`; requirement planners stay pure once current state is supplied.
