...
export namespace Payload {
   ...
   * @throws {PayloadDecodeError} when the item list is empty, encoding or compression
  ...
  export async function encode(items: readonly Item[]): Promise<Hex> {
    if (CURRENT_VERSION > 0xff) {
      throw new PayloadDecodeError(
        `version overflow: ${CURRENT_VERSION} exceeds 255`,
      );
    }
   ...
   * @throws {PayloadDecodeError} when the payload is malformed or exceeds protocol limits.
   * @example
   * ```ts
   * import { Offer, Payload } from "@morpho-org/midnight-sdk";
   * import { zeroAddress } from "viem";
  ...
  export async function decode(
    ...
    if (compressedEnd > bytes.length) {
      throw new PayloadDecodeError(
        "payload truncated before declared gzip length",
      );
    }
  ...
  function decodeItemsBytes(decoded: Uint8Array): Item[] {
    ...
    try {
      ...
      if (error instanceof Error) {
        throw new PayloadDecodeError(
          `invalid offer bytes: ${error.message}`,
          error,
        );
      }
      throw error;
  ...
  function assertCanonicalItemsBytes(
    ...
    if (canonical !== bytesToHex(decoded)) {
      throw new PayloadDecodeError("items payload has non-canonical ABI bytes");
    }
  }
  ...
  function assertNonEmptyItemCount(count: number): void {
    if (count === 0) throw new PayloadDecodeError("items payload is empty");
  }
  ...
  function assertCallerItemCountLimit(
    ...
    if (count > maxItems) {
      throw new PayloadDecodeError(`items payload exceeds ${maxItems} items`);
    }
  }

  async function compressItemsPayload(
    payload: Uint8Array,
  ): Promise<Uint8Array> {
  ...
  async function decompressItemsPayload(
    ...
    if (compressed.length === 0)
      throw new PayloadDecodeError("decompression failed");
    if (compressed.length > MAX_COMPRESSED_ITEMS_BYTES) {
      throw new PayloadDecodeError(
        `compressed items exceed ${MAX_COMPRESSED_ITEMS_BYTES} bytes`,
      );
  ...
  function assertAbiItemCountWithinCallerLimit(
    ...
    if (decoded.length < ABI_ARRAY_HEAD_BYTES) {
      throw new PayloadDecodeError(
        "items payload truncated before ABI array head",
      );
    }
    if (maxItems === undefined) return;
