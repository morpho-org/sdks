/**
 * Thrown when a bigint edge receives an unsafe JavaScript number.
 *
 * @example
 * ```ts
 * import { UnsafeNumberError } from "@morpho-org/midnight-sdk";
 *
 * try {
 *   throw new UnsafeNumberError("assets", Number.MAX_SAFE_INTEGER + 1);
 * } catch (error) {
 *   console.log(error instanceof UnsafeNumberError);
 * }
 * ```
 */
export class UnsafeNumberError extends Error {
  public constructor(field: string, value: number) {
    super(
      `${field} must be a safe integer; pass a bigint for value "${value}".`,
    );
...
/**
 * Thrown when a bigint edge receives a value that cannot be parsed as an integer.
 *
 * @example
 * ```ts
 * import { InvalidMidnightBigIntError } from "@morpho-org/midnight-sdk";
 *
 * throw new InvalidMidnightBigIntError({
 *   field: "assets",
 *   value: "not-a-number",
 * });
 * ```
 */
export class InvalidMidnightBigIntError extends Error {
  public constructor(params: {
    ...
    super(
      `${params.field} must be a bigint-compatible integer, got "${params.value}".`,
      {
        cause: params.cause,
      },
    );
    this.name = "InvalidMidnightBigIntError";
  }
}

/**
 * Thrown when an address input is not an EVM address.
 *
 * @example
 * ```ts
 * import { InvalidMidnightAddressError } from "@morpho-org/midnight-sdk";
 *
 * throw new InvalidMidnightAddressError("maker", "not-an-address");
 * ```
 */
export class InvalidMidnightAddressError extends Error {
  public constructor(field: string, value: string) {
    super(`${field} must be an EVM address, got "${value}".`);
    this.name = "InvalidMidnightAddressError";
  }
}

/**
 * Thrown when a hex input is malformed or has the wrong byte length.
 *
 * @example
 * ```ts
 * import { InvalidMidnightHexError } from "@morpho-org/midnight-sdk";
 *
 ...
 * ```
 */
export class InvalidMidnightHexError extends Error {
  public constructor(params: {
    readonly field: string;
    ...
    super(
      `${params.field} must be a ${params.expected ?? "hex string"}, got "${params.value}".`,
    );
    this.name = "InvalidMidnightHexError";
  }
}
