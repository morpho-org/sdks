...
export const registerWorkerFixture = async (name: "worker-a" | "worker-b") => {
  const coordinationDirectory =
    process.env.MORPHO_TEST_SEMAPHORE_COORDINATION_DIRECTORY!;
  await writeFile(
    join(coordinationDirectory, `${name}.ready`),
    `${process.pid}\n`,
  );
  ...
  test(`${name} acquires the shared slot`, async ({ client }) => {
    ...
    await writeFile(join(coordinationDirectory, `${name}.done`), "acquired\n");
  });
};
