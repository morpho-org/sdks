...
await writeFile(
  join(coordinationDirectory, "worker-a.ready"),
  `${process.pid}\n`,
);
...
test("worker A acquires the shared slot", async ({ client }) => {
  ...
  await writeFile(join(coordinationDirectory, "worker-a.done"), "acquired\n");
});
