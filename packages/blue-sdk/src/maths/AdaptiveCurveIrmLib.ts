...
export namespace AdaptiveCurveIrmLib {
   ...
   */
  export const WEXP_UPPER_VALUE = 57716089161558943949701069502944508345128_422502756744429568n;
  ...
  export function getBorrowRate(startUtilization: BigIntish, startRateAtTarget: BigIntish, elapsed: BigIntish) {
    startUtilization = BigInt(startUtilization);
    startRateAtTarget = BigInt(startRateAtTarget);
    elapsed = BigInt(elapsed);

    const errNormFactor = startUtilization > TARGET_UTILIZATION ? MathLib.WAD - TARGET_UTILIZATION : TARGET_UTILIZATION;
    const err = MathLib.wDivDown(startUtilization - TARGET_UTILIZATION, errNormFactor);
    ...
    if (startRateAtTarget === 0n) {
      ...
      if (linearAdaptation === 0n) {
        ...
        const _newRateAtTarget = (linearAdaptation: BigIntish) =>
          MathLib.min(
            MathLib.max(MathLib.wMulDown(startRateAtTarget, wExp(linearAdaptation)), MIN_RATE_AT_TARGET),
            MAX_RATE_AT_TARGET
          );
        ...
        avgRateAtTarget = (startRateAtTarget + endRateAtTarget + 2n * _newRateAtTarget(linearAdaptation / 2n)) / 4n;
      }
    }

    // Non negative because 1 - 1/C >= 0, C - 1 >= 0.
    const coeff =
      err < 0 ? MathLib.WAD - MathLib.wDivDown(MathLib.WAD, CURVE_STEEPNESS) : CURVE_STEEPNESS - MathLib.WAD;
