...
export namespace MarketUtils {
  ...
  export function getLiquidationIncentiveFactor({ lltv }: { lltv: BigIntish }) {
    return MathLib.min(
      MAX_LIQUIDATION_INCENTIVE_FACTOR,
      MathLib.wDivDown(MathLib.WAD, MathLib.WAD - MathLib.wMulDown(LIQUIDATION_CURSOR, MathLib.WAD - BigInt(lltv)))
    );
  }
  ...
  export function getCollateralPower(collateral: BigIntish, { lltv }: { lltv: BigIntish }) {
    return MathLib.wMulDown(collateral, lltv);
  }

  export function getCollateralValue(collateral: BigIntish, { price }: { price: BigIntish }) {
  ...
  export function getMaxBorrowableAssets(
    { collateral, borrowShares }: { collateral: BigIntish; borrowShares: BigIntish },
    market: {
      totalBorrowAssets: BigIntish;
  ...
  export function getLiquidationSeizedAssets(
    ...
    return MathLib.mulDivDown(
      MathLib.wMulDown(toBorrowAssets(repaidShares, market, "Down"), getLiquidationIncentiveFactor(config)),
      ORACLE_PRICE_SCALE,
      market.price
    );
  }
  ...
  export function getWithdrawableCollateral(
    ...
    return MathLib.zeroFloorSub(
      collateral,
      MathLib.wDivUp(MathLib.mulDivUp(toBorrowAssets(borrowShares, market), ORACLE_PRICE_SCALE, market.price), lltv)
    );
  }
  ...
  export function isHealthy(
    ...
    return getMaxBorrowAssets(collateral, market, marketConfig) >= toBorrowAssets(borrowShares, market);
  }
