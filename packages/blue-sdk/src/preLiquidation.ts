...
import { PreLiquidationParams } from "./position/PreLiquidationPosition";
import type { BigIntish } from "./types";

export const preLiquidationParamsRegistry = new Map([
  [
    parseEther("0.385"),
    new PreLiquidationParams({
...
export const getPreLiquidationParams = (lltv: BigIntish) => {
  lltv = BigInt(lltv);

  const params = preLiquidationParamsRegistry.get(lltv);

  if (params == null) throw new UnsupportedPreLiquidationParamsError(lltv);
