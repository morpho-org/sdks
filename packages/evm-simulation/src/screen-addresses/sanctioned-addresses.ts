/**
 * Static list of sanctioned/compromised addresses.
 * Ported from offchain-services-server/src/functions/screen/blacklist.ts.
 * These addresses are checked before calling the Chainalysis API (fast, no network).
 *
 * **Invariant** (enforced at module load, see below): every entry is a
 * well-formed lowercased EVM address. `screenAddresses` does
 * `sanctionedAddresses.has(addr.toLowerCase())` — a mixed-case entry would
 * never match and would silently bypass the sanctioned check.
 */
export const sanctionedAddresses: Set<string> = new Set(
  [
...
// Runtime invariant check at module load. Catches a future edit that adds an
// entry via `sanctionedAddresses.add(...)` bypassing the `.map(toLowerCase)` or
