...
describe("parseTransfers scenarios: vault V2 deposit", () => {
  ...
  const depositLogs: RawLog[] = [
    {
      address: USDC,
      topics: [TRANSFER_TOPIC, padAddress(USER), padAddress(ADAPTER)],
      data: encodeUint256(1_000_000n), // 1 USDC
    },
    {
      address: USDC,
      topics: [TRANSFER_TOPIC, padAddress(ADAPTER), padAddress(bbqUSDC)],
      data: encodeUint256(1_000_000n),
    },
    {
      address: bbqUSDC,
      topics: [TRANSFER_TOPIC, `0x${"0".repeat(64)}` as Hex, padAddress(USER)],
      data: encodeUint256(950_000_000_000_000_000n), // shares minted
    },
  ];
...
describe("parseTransfers scenarios: WETH wrap/unwrap", () => {
...
describe("parseTransfers scenarios: multi-token market operation", () => {
  ...
  it("emits both collateral-in and borrow-out transfers with correct amounts", () => {
    const logs: RawLog[] = [
      ...
      {
        address: WETH,
        topics: [TRANSFER_TOPIC, padAddress(USER), padAddress(MORPHO)],
        data: encodeUint256(1_000_000_000_000_000_000n), // 1 WETH
      },
      // USDC: Morpho → user (borrow)
      {
        address: USDC,
        topics: [TRANSFER_TOPIC, padAddress(MORPHO), padAddress(USER)],
        data: encodeUint256(2_000_000_000n), // 2000 USDC
      },
    ];
...
describe("parseTransfers scenarios: determinism", () => {
  ...
  it("same logs produce same output regardless of input order", () => {
    ...
    const log1: RawLog = {
      address: A,
      topics: [TRANSFER_TOPIC, padAddress(B), padAddress(C)],
      data: encodeUint256(100n),
    };
    const log2: RawLog = {
      address: C,
      topics: [TRANSFER_TOPIC, padAddress(A), padAddress(B)],
      data: encodeUint256(200n),
    };
