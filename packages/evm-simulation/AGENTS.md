...
- The full v5 pipeline runs `parseRequest → decodeAndBind → resolvePinnedBlock → readPinnedInputs → checkRequests → planProbeReads → planExecution → executePlan → proveAuthorizations → verifyEffects → enforceLimits → assembleResult`. Preparation calls and probes carry `preparation`/`probe` identities and never appear in `simulationTxs`/`calls`/`transfers` — only user transactions carry public `txIdx`.
