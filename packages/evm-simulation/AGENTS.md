...
- Enforce retention by net `(restricted address, token)` balance across the blue-sdk `bundler3` and `bundles` registries with `DUST_THRESHOLD = 100n`; skip only chains that catalog neither. The guard runs on the *simulated* bundle, which executes at a non-zero gas price (the caller's fee, or `DEFAULT_SIMULATION_GAS_PRICE` — never `0`, an impossible on-chain value that would hide a fee-sensitive `skipRevert: true` revert, Cantina 1631). Resolve the fee context once in `resolveFeeContext` and serialize it in every backend; never reintroduce a zero-fee default. Input validation rejects a zero/negative or legacy+1559-mixed fee — on caller and `approval`-authorization transactions alike — so an invalid fee fails as `SimulationValidationError`, not a backend `ExternalServiceError`. Builders still keep every value-carrying step `skipRevert: false` so the bundle reverts atomically.
