...
export const safeGetAddress = (address: string) => getAddress(address.toLowerCase());

export const safeParseNumber = (value: number, decimals = 18) => safeParseUnits(toFixed(value, decimals), decimals);

export const safeParseUnits = (strValue: string, decimals = 18) => {
  if (!/[-+]?[0-9]*\.?[0-9]+/.test(strValue)) throw new Error("invalid number: " + strValue);
...
): address is "0x0000000000000000000000000000000000000000" => address == null || address === ZeroAddress;
