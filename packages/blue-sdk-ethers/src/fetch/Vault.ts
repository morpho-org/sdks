...
export async function fetchVaultFromConfig(
  ...
  const [
    ...
    mm.pendingTimelock(overrides).then(({ value, validAt }) => ({ value, validAt })),
    mm.pendingGuardian(overrides).then(({ value, validAt }) => ({ value, validAt })),
    ...
    chainAddresses.publicAllocator && mm.isAllocator(chainAddresses.publicAllocator, overrides),
  ]);

  let publicAllocatorConfigPromise: Promise<VaultPublicAllocatorConfig> | undefined;

  if (hasPublicAllocator) {
    const publicAllocator = PublicAllocator__factory.connect(chainAddresses.publicAllocator!, runner);
  ...
  const [supplyQueue, withdrawQueue, publicAllocatorConfig] = await Promise.all([
    Promise.all(new Array(supplyQueueSize).fill(null).map((_, i) => mm.supplyQueue(i, overrides) as Promise<MarketId>)),
    Promise.all(
      new Array(withdrawQueueSize).fill(null).map((_, i) => mm.withdrawQueue(i, overrides) as Promise<MarketId>)
    ),
...
export async function fetchAccrualVault(
  ...
  const allocations = await Promise.all(
    [...new Set(vault.supplyQueue.concat(vault.withdrawQueue))].map((marketId) =>
