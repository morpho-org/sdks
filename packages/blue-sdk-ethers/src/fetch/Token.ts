import { Provider, Contract, ContractRunner, Interface, decodeBytes32String, isHexString } from "ethers";
...
const _bytes32ERC20Abi = [
  ...ERC20__factory.abi.filter(
    (fragment) => !("name" in fragment) || (fragment.name !== "name" && fragment.name !== "symbol")
  ),
  {
    inputs: [],
    name: "symbol",
...
export class Bytes32ERC20__factory {
  ...
  static connect(address: string, runner?: ContractRunner | null) {
    const erc20 = new Contract(address, _bytes32ERC20Abi, runner) as unknown as ERC20;

    const name = erc20.name.bind(erc20);
    erc20.name = Object.assign((...args: Parameters<typeof name>) => name(...args).then(decodeString), name);

    const symbol = erc20.symbol.bind(erc20);
    erc20.symbol = Object.assign((...args: Parameters<typeof symbol>) => symbol(...args).then(decodeString), symbol);
...
export class ERC20Metadata__factory {
  static connect(address: string, chainId: ChainId, runner?: ContractRunner | null) {
    if (isBytes32ERC20Metadata(address, chainId)) return Bytes32ERC20__factory.connect(address, runner);
    ...
    erc20.name = Object.assign(
      (...args: Parameters<typeof name>) =>
        name(...args).catch(() => Bytes32ERC20__factory.connect(address, runner).name(...args)),
      name
    );
    ...
    erc20.symbol = Object.assign(
      (...args: Parameters<typeof symbol>) =>
        symbol(...args).catch(() => Bytes32ERC20__factory.connect(address, runner).symbol(...args)),
      symbol
    );
