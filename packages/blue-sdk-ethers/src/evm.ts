import { TransactionRequest, Signer, parseUnits, TransactionResponse } from "ethers";

import { ChainId, MathLib } from "@morpho-org/blue-sdk";

import { NotificationProducer, NotificationStatus, NotificationTopic } from "./notifications";
...
export const sendTransactionWithProducer = async (
  ...
  try {
    let gasLimit: bigint;
    try {
      gasLimit = MathLib.wMulUp(await signer.estimateGas(req), parseUnits("1.1"));
    } catch (error: any) {
...
export const signMessageWithProducer = async (
  ...
  try {
    ...
    SignatureUtils.verifySignature(signature, message.hash, await signer.getAddress());
