...
**Scope / limits.** Exact pinning is a release-hygiene choice, not a security control against a malicious re-publish of an already-released version — that is defended downstream by committed lockfiles, integrity digests, `minimumReleaseAgeStrict`, and publish provenance. This package lets `morpho-sdk`/`blue-sdk` typed errors propagate uncaught; a consuming app that also installs those packages directly at a different version could end up with duplicate copies, breaking `instanceof` on errors that cross this boundary. Prefer resolving both to a single version via the app lockfile.
