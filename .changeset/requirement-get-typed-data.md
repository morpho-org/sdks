...
fixtures, need not supply it) and is deep-frozen. Obtaining the signature via `action.typedData`
skips `sign()`'s recover-and-verify step, so the caller is responsible for verification. For the
Midnight offer-root requirement, `sign()` additionally derives the ratification payload consumed by
`buildTx()`, so remote signers should be wrapped in a viem custom account and passed to `sign()`
rather than signing `typedData` directly.
