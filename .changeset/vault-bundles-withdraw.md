...
Low-level `vaultV1Withdraw` and `vaultV2Withdraw` builders require explicit `VaultWithdrawalAuthorization`: either `{ type: "allowance" }` or `{ type: "permit", signature, shareAllowance }`. Permit authorization validates the independently supplied exact share allowance, vault, owner, spender, deadline, and nonce metadata. Entity withdrawals supply their captured share cap automatically. Replace the unreleased `getBundlesSharesPermit` helper with `emptySharesPermit`, `validateSharesPermit`, and `toSharesPermitStruct`, separating sentinel construction, required validation, and struct conversion.
