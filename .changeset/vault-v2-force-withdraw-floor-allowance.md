...
- The approved / permitted share allowance now covers a burn at least one `slippageTolerance` step below the floor and, in any case, at least one share above the burn at the floor, so a price within that headroom below the floor reverts on the bundle's `minSharePriceE27` check rather than underflowing `_spendAllowance` (`panic 0x11`) on the final `withdraw` leg; a deeper drop can still surface as an ERC-20 allowance underflow. Still saturated at `maxUint256`.
