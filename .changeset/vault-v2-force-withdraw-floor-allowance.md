...
- The approved / permitted share allowance now covers a burn one `slippageTolerance` step below the floor, and always at least one RAY price unit below it when the floor exceeds 1, so `slippageTolerance: 0n` no longer sizes the allowance exactly to the floor (still saturated at `maxUint256`): a below-floor price reverts on the bundle's `minSharePriceE27` check instead of underflowing `_spendAllowance` (`panic 0x11`) on the final `withdraw` leg.
