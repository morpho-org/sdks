...
Accrue the contributing underlying adapters, markets, and positions when accruing a Vault V2, so `AccrualVaultV2.accrueInterest` returns an entity graph whose asset-bearing nested state shares one `lastUpdate` — aligning Vault V2 with the existing MetaMorpho V1 (`AccrualVault.accrueInterest`) behavior, rather than leaving nested adapters at pre-accrual state. Adds a backward-compatible optional `accrueInterest(timestamp)` method to the `IAccrualVaultV2Adapter` interface — adapters that do not implement it are left at their pre-accrual state — and implements it on each built-in adapter. Non-contributing nested state is intentionally not accrued (see below), so callers must not read it as part of the shared snapshot. Vault-level `_totalAssets`, `totalSupply`, and fee shares are byte-for-byte unchanged.
