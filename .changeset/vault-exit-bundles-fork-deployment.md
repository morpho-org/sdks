...
pure function of the bound Morpho Blue address and the salt rather than of the deployer's nonce —
which keeps it stable across forks and makes registering it with `registerCustomAddresses`
idempotent. Deploying twice returns the existing address instead of reverting.
